...
class MotChallenge2DBox(_BaseDataset):
    ...
    def __init__(self, config=None):
        ...
        # Get classes to eval
        self.valid_classes = ['pedestrian', 'vehicle']
        self.class_list = [cls.lower() if cls.lower() in self.valid_classes else None
                           for cls in self.config['CLASSES_TO_EVAL']]
        if not all(self.class_list):
            raise TrackEvalException('Attempted to evaluate an invalid class. Valid classes: pedestrian, vehicle')
        ...
        if self.config["SEQ_INFO"]:
            ...
            with open(seqmap_file) as fp:
                ...
                for row in reader:
                    if not row or row[0] == '':
                        continue
                    if row[0].strip().lower() in ['sequence', 'seq', 'seqname']:
                        continue
                    seq = row[0].strip()
                    seq_list.append(seq)
                    ini_file = os.path.join(self.gt_fol, seq, 'seqinfo.ini')
